...
class TeacherStates(StatesGroup):
    ...
    waiting_for_custom_date = State()  # Добавляем новое состояние
...
class GroupStates(StatesGroup):
    waiting_for_custom_date = State()  # Добавляем новое состояние для групп

@router.callback_query(F.data.startswith('day_custom_'))
async def handle_custom_date_group(callback: CallbackQuery, state: FSMContext):
...
@router.message(GroupStates.waiting_for_custom_date)
async def process_group_custom_date(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            f"📅 Расписание группы {group_name} на {message.text}:\n\n{schedule_text}",
            parse_mode="HTML"
        )
        await state.clear()
        ...
        logger.error(f"Error in custom date handler: {e}", exc_info=True)
        await message.answer("❌ Ошибка при получении расписания")
        await state.clear()

@router.message(TeacherStates.waiting_for_custom_date)
async def process_teacher_custom_date(message: Message, state: FSMContext):
    try:
        date_obj = datetime.strptime(message.text, '%d.%m.%Y').date()
        date_str = date_obj.strftime('%Y-%m-%d')
        ...
        if found_pairs:
            ...
            response = f"<b>📅 Расписание преподавателя {teacher_name} на {message.text}:</b>\n\n{schedule_text}"
        else:
            response = f"❌ Преподаватель {teacher_name} не найден в расписании на {message.text}."
        ...
        logger.error(f"Error in teacher custom date handler: {e}", exc_info=True)
        await message.answer("❌ Ошибка при получении расписания")
        await state.clear()

@router.message(ClassroomStates.waiting_for_custom_date)
async def process_classroom_custom_date(message: Message, state: FSMContext):
    try:
        date_obj = datetime.strptime(message.text, '%d.%m.%Y').date()
        date_str = date_obj.strftime('%Y-%m-%d')
        ...
        for group in all_groups:
            query_params = {"date": date_str, "group": group, "teacher": ""}
            if (cached_data := get_cached(query_params)):
                cached_pairs.extend(
        ...
        # Делаем запрос для отсутствующих данных
        found_pairs = []
        if need_fetch:
            async with APIClient() as client:
                batch_data = await client.fetch_batch_schedules(
        ...
        if all_pairs:
            ...
            response = f"<b>📅 Расписание аудитории {classroom_name} на {message.text}:</b>\n\n{schedule_text}"
        else:
            response = f"✅ Аудитория <b>{classroom_name}</b> свободна на {message.text}."
        ...
        logger.error(f"Error in classroom custom date handler: {e}", exc_info=True)
        await message.answer("❌ Ошибка при получении расписания")
        await state.clear()
...
async def ask_for_period(message: Message, entity_name: str, state: FSMContext, entity_type: str):
    """Универсальная функция для запроса периода"""
    builder = InlineKeyboardBuilder()
    builder.add(
        InlineKeyboardButton(text="Сегодня", callback_data=f"{entity_type}_today_{entity_name}"),
        InlineKeyboardButton(text="Завтра", callback_data=f"{entity_type}_tomorrow_{entity_name}"),
        InlineKeyboardButton(text="Неделя", callback_data=f"{entity_type}_week_{entity_name}"),
        InlineKeyboardButton(text="Другая дата", callback_data=f"{entity_type}_custom_{entity_name}"),
    )
    builder.adjust(2, 2)
...
async def process_teacher_name(message: Message, state: FSMContext):
    ...
    await ask_for_period(message, teacher_name, state, 'teacher')

#Обработчик выбора периода для преподавателя
@router.callback_query(F.data.startswith('teacher_'))
async def process_teacher_period(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
...
async def process_classroom_name(message: Message, state: FSMContext):
    ...
    # Заменяем ручное создание клавиатуры на вызов универсальной функции
    await ask_for_period(
        message=message,
