...
# Добавим новое состояние для выбора периода
class TeacherStates(StatesGroup):
    waiting_for_teacher_name = State()
    waiting_for_period = State()  # Новое состояние для выбора периода

# Измененный обработчик кнопки "Преподаватель"
...
# Измененный обработчик ввода имени преподавателя
...
# Новый обработчик для выбранного периода
...
async def process_classroom_name(message: Message, state: FSMContext):
    """Обработчик ввода номера аудитории"""
...
async def ask_for_classroom_period(message: Message, classroom_name: str, state: FSMContext):
    """Запрашивает период для поиска аудитории"""
    builder = InlineKeyboardBuilder()
    builder.add(
